...
bool LlvmCodeGen::generate_abi_header(const std::string& output_path) {
    ...
    if (!out.is_open()) {
        error_message = "Cannot open output file: " + output_path;
        return false;
    }

    // Write C ABI type mappings
    out << "// Novium SDK Header\n";
    out << "// Auto-generated by `novium build --header` — do not edit by hand\n\n";

    out << "// C ABI type mappings\n";
    out << "#define NOVIUM_C_TYPE(int)     int32_t\n";
    out << "#define NOVIUM_C_TYPE(int64)  int64_t\n";
    out << "#define NOVIUM_C_TYPE(float)   float\n";
    out << "#define NOVIUM_C_TYPE(double)  double\n";
    out << "#define NOVIUM_C_TYPE(bool)    _Bool\n\n";

    out << "// String / slice types: { const char*; size_t }\n";
    out << "#define NOVIUM_C_TYPE(string) { const char*; size_t }\n";
    out << "#define NOVIUM_C_TYPE(slice) { void*; size_t }\n\n";

    out << "// Version info\n";
    out << "#define NOVIUM_SDK_VERSION \"0.2.0\"\n\n";

    out << "// Build configuration\n";
    out << "#define NOVIUM_SDK_PROFILE \"debug\"\n";
    out << "#define NOVIUM_SDK_TARGET \"native\"\n\n";

    out << "// Function declarations for SDK runtime (C ABI)\n";
    out << "extern void* novium_malloc(size_t size);\n";
    out << "extern void novium_free(void* ptr);\n";
    out << "extern void novium_print(const char* s);\n";
    out << "extern int novium_add(int a, int b);\n\n";

    out << "// Convenience macros for type usage in FFI\n";
    out << "#define NOVIUM_INT int32_t\n\n";

    out << "// Novium string type for C FFI (pointer + length)\n";
    out << "typedef struct {\n";
    out << "    const char* data;\n";
    out << "    size_t length;\n";
    out << "} NoviumString;\n\n";

    out << "// Novium slice type for C FFI\n";
    out << "typedef struct {\n";
    out << "    const char* data;\n";
    out << "    size_t length;\n";
    out << "} NoviumSlice;\n\n";

    out << "// Null pointer macro\n";
    out << "#define NOVIUM_NULL ((void*)0)\n";
...
void LlvmCodeGen::generate_module(const std::vector<std::unique_ptr<novium::Stmt>>& program) {
    ...
    // Run LLVM optimization pipeline based on config
    if (config_.optimization_level == CodeGenConfig::OptimizationLevel::AGGRESSIVE) {
        // Run full optimization pipeline
        llvm::PassBuilder pb;
        
        // Register standard LPNH passes
        pb.registerPipelineParsingCallback(
            [](llvm::StringRef Name, llvm::ArrayRef<llvm::PassBuilder::PipelineElement>,
               llvm::EarlyStartPipe* = nullptr) -> bool {
                return false;
            });
        
        if (config_.emit_debug_info) {
            // With debug info, run verified + optimize size
            llvm::verifyModule(*module_, llvm::VerifierOptions());
            llvm::legacy::PassManager PM;
            PM.addPass(llvm::createInstructionCombiningPass());
            PM.addPass(llvm::createGVNPass());
            PM.addPass(llvm::createCFGSimplificationPass());
            PM.run(*module_);
        } else {
            // Full optimization without debug info constraints
            llvm::verifyModule(*module_, llvm::VerifierOptions());
            llvm::legacy::PassManager PM;
            PM.addPass(llvm::createInstructionCombiningPass());
            PM.addPass(llvm::createGVNPass());
            PM.addPass(llvm::createCFGSimplificationPass());
            PM.addPass(llvm::createDeadStoreEliminationPass());
            PM.addPass(llvm::createDeadCodeEliminationPass());
            PM.run(*module_);
        }
    } else if (config_.optimization_level == CodeGenConfig::OptimizationLevel::BASIC) {
        // Basic optimization: instruction combining only
        llvm::verifyModule(*module_, llvm::VerifierOptions());
        llvm::legacy::PassManager PM;
        PM.addPass(llvm::createInstructionCombiningPass());
        PM.run(*module_);
    } else {
        // No optimization (NONE level) - just verify
        llvm::verifyModule(*module_, llvm::VerifierOptions());
    }
    
    // Set target triple based on hardware target for GPU code generation
    if (fn->hardware_target == HardwareTarget::CUDA) {
        module_->get()->setTargetTriple("nvidia64-nvidia-cuda");
        llvm_fn->setAttr(llvm::Attribute::GPU);
    } else if (fn->hardware_target == HardwareTarget::ROCm) {
        module_->get()->setTargetTriple "amd-amdgcn-amdhsa");
        llvm_fn->setAttr(llvm::Attribute::GPU);
    } else if (fn->hardware_target == HardwareTarget::ONEAPI) {
        module_->get()->setTargetTriple "intel-spir");
        llvm_fn->setAttr(llvm::Attribute::GPU);
    }
}
...
void* LlvmCodeGen::get_llvm_type(novium::TypePtr type) {
    ...
    switch (type->kind) {
        ...
        case novium::TypeKind::INT8:  return llvm::IntegerType::getInt8(*ctx_->context_);
        case novium::TypeKind::INT16: return llvm::IntegerType::getInt16(*ctx_->context_);
        case novium::TypeKind::INT32: return llvm::IntegerType::getInt32(*ctx_->context_);
        case novium::TypeKind::INT64: return llvm::IntegerType::getInt64(*ctx_->context_);
        case novium::TypeKind::UINT8:  return llvm::IntegerType::getInt8(*ctx_->context_);
        case novium::TypeKind::UINT16: return llvm::IntegerType::getInt16(*ctx_->context_);
        case novium::TypeKind::UINT32: return llvm::IntegerType::getInt32(*ctx_->context_);
        case novium::TypeKind::UINT64: return llvm::IntegerType::getInt64(*ctx_->context_);
        case novium::TypeKind::FLOAT: return llvm::FloatType::get(*ctx_->context_);
        case novium::TypeKind::FLOAT16: return llvm::HalfType::get(*ctx_->context_);
        case novium::TypeKind::FLOAT32: return llvm::FloatType::get(*ctx_->context_);
        case novium::TypeKind::FLOAT64: return llvm::DoubleType::get(*ctx_->context_);
        ...
        case novium::TypeKind::TENSOR: {
            // Tensor is lowered to a pointer to its data buffer
            // Shape and dtype are metadata; the actual data is a flat buffer
            return llvm::PointerType::getInt8Ptr(*ctx_->context_);
        }
        case novium::TypeKind::MATRIX: {
            // Matrix is lowered to a pointer to its data buffer
            return llvm::PointerType::getInt8Ptr(*ctx_->context_);
        }
        ...
        case novium::TypeKind::INFER: {
            // Inferred type - use i32 as default (better than i64 for most cases)
            return llvm::IntegerType::getInt32(*ctx_->context_);
        }
        default:
            return llvm::IntegerType::getInt32(*ctx_->context_);
    }
...
void* LlvmCodeGen::codegen_literal(novium::LiteralExpr* expr) {
    ...
    switch (expr->token.type) {
        case novium::TokenType::INTEGER_LITERAL: {
            ...
            // Use i32 default to match updated type system
            int_val = llvm::ConstantInt::get(*ctx_->context_, llvm::APInt(32, val));
            return int_val;
        }
